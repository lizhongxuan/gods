	...
	if srcInFront {
		if src.isLeaf == false {
			for j < eNum {
				...
				tree.RemoveElement(false, src, Child, src.KeyNum-1, INT_MIN)
				//每删除一个节点keyNum也自动减少1 队尾删
				tree.InsertElement(false, dst, Child, INT_MIN, 0, INT_MIN, INT_MIN)
				//队头加
				j++
			}
		} else {
		...
		/* 将树叶节点重新连接 */
		if src.KeyNum > 0 {
			FindMostRight(src).leafNode.Next = FindMostLeft(dst)
			//似乎不需要重连，src的最右本身就是dst最左的上一元素
		} else {
			if src.isLeaf == true {
...
func (tree *BPlusTree) MergeNode(Parent Position, X Position, S Position, i int) Position {
	...
	if S.KeyNum > LIMIT_M_2 {
		...
		/* 将X全部元素移动到S中，并把X删除 */
		Limit = X.KeyNum
		tree.MoveElement(X, S, Parent, i, Limit)
		//最多时S恰好MAX MoveElement已考虑了parent.key的索引更新
...
func (tree *BPlusTree) RecursiveInsert(beInsertedElement Position, Key int, posAtParent int, Parent Position, data int) (Position, bool) {
	...
	if beInsertedElement.isLeaf == true {
		beInsertedElement = tree.InsertElement(true, Parent, beInsertedElement, Key, posAtParent, InsertIndex, data)
		//返回叶子节点
		/* 内部节点 */
	} else {
